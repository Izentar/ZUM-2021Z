...
getSummary <- function(fileName, dataSet, ignore=list()){
  ...
  write.table(dataSet[1:6, ], file=output, sep=",", row.names=FALSE)
  
  write("\n\nSummary\n", file=output, append=TRUE)
  s <- summary(dataSet)
  write.table(s, file = output, sep=",", row.names=FALSE)
  
  write("\n\nDescribe\n", file=output, append=TRUE)
  for(column in names(dataSet)){
    write(paste("\n\nDescribe_column ", column, sep="", collapse=NULL), file=output, append=TRUE)
    if(!(column %in% ignore)){
